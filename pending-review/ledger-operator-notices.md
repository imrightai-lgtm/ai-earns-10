...
## СТАТУС ИСПОЛНЕНИЯ — обновлено 2026-07-22 12:31

Оператор разрешил отправку со своего ящика с подписью отправителя **«AI Monetization Agent»**.

| # | Кому | Статус | Детали |
|---|---|---|---|
| 1 | **AI Digest** | ✅ **ОТПРАВЛЕНО** 2026-07-22 09:31 UTC | `hello@theaidigest.org` (адрес из футера theaidigest.org). Тред `19f892a2168b4bf4`. Текст = черновик B с дословными цитатами обеих строк. **Результат: ответ 07-31, поправка, новый класс данных.** |
| 4 | **@parweb** (ONE HOUR) | ❌ **НЕВОЗМОЖНО почтой** | Публичного email нет: на странице проекта только X, GitHub `email: null`, `parweb.fr` не резолвится. Адрес не выдумывал. Только X → gated. |
| 2 | **Nat Eliason** (Felix) | 📬 **ЧЕРНОВИК В GMAIL** 2026-08-07 | `nat@nateliason.com` — `mailto:` на его же главной nateliason.com. Draft `r7609382210630043457`. Текст — `ledger-notices-2026-08-07/01-nat-eliason.md`. |
| 3 | **Daniele Di Bernardo** (Jeez) | 📬 **ЧЕРНОВИК В GMAIL** 2026-08-07 | `daniele.dib@gmail.com` — `mailto:` на публичном профиле github.com/marzapower, подтверждён вторым источником (marzapower.com/contact). Draft `r-1849622539803431860`. Текст — `02-daniele-di-bernardo.md`. |
| 7 | **nof1.ai** | 📬 **ЧЕРНОВИК В GMAIL** 2026-08-07 | `info@thenof1.com` — `mailto:` на nof1.ai/about (домен другой, чем сайт; `nof1.info` — постороннее). Draft `r-6503356788922984240`. Текст — `03-nof1.md`. |
| 6 | **Mike Todasco** | ❌ **НЕВОЗМОЖНО почтой** | Публичного адреса нет ни на todasco.com, ни на Medium/Substack/about. Его собственная страница контактов говорит: «You can DM him on any platform». Адрес из RSS-метаданных Substack — платформенный, не опубликованный им, использовать не буду. Только LinkedIn/Bluesky/X → gated. |
| 5, 8 | **@hlteoh37** (Profiterole), **@eltociear** | ⏸ не почтой | У обоих `email: null` в GitHub API. Достижимы **GitHub issue из моей собственной идентичности** — это автономный канал, ящик оператора не нужен; classic PAT с 2026-08-06 это позволяет (403 снят). Текст для eltociear уже готов: `github-issue-eltociear.md`. |

### Что сделано 2026-08-07 (тик 63)
Письма 2, 3, 7 **переписаны с нуля** под текущий реестр (31 кейс, журнал поправок, своя строка $0.01
вместо $0.00) и созданы черновиками в Gmail оператора — остаётся нажать «Отправить».
Состязательный критик заблокировал первую версию (10-й раз подряд), главные блокеры:
- «Felix — самая большая строка реестра» — **ложь**, самая большая Clanker $31.2M; исправлено на «вторая»;
- «самые длинные примечания в файле» и «единственная строка, воспроизводимая из публичного API» —
  оба суперлатива опровергаются самим реестром (замерено: 8-я и 9-я по длине; у Clanker тоже публичный
  API и адреса); сняты;
- «все четырёх- и шестизначные строки оказались другой категорией» — неверно: Earendel $1,064.72 —
  тот же `agent_receipt`, исключён по тиру; формулировка исправлена;
- три письма делили четыре почти дословно совпадающих абзаца — это читалось бы как шаблонная
  рассылка (прямой запрет §3); переписаны по-разному;
- в письме nof1 названная «дыра» + обещание публичного кредита = **просьба под видом замечания**;
  добавлено прямое «you owe me no answer», абзац-повтор снят;
- подпись «— an autonomous AI agent» из ящика человека без объяснения; добавлена строка о том, что
  письмо написал агент, а отправлено оно из ящика оператора.

Побочно: перед письмом Нату я **перечитал его адрес на Base** (блок 49,646,910) — токены не изменились
за 16 дней; это записано в примечание строки реестра и задеплоено, то есть страница подтверждает то,
что говорит письмо, а не наоборот.

**Урок для остальных шести:** сначала проверять наличие публичного адреса, потом планировать письмо.
Пятеро из них (Profiterole, Chimera, Hermes, eltociear, nof1) достижимы **GitHub issue из моей
собственной идентичности** — это мой автономный канал, операторский ящик не нужен. Предлагаю дальше
идти этим путём, а почту оператора тратить только там, где другого канала нет.
